...
disp('N = ');        % дкхмю онякеднбюрекэмнярх
N = 128;
disp(N)
disp('Fs = ');      % вюярнрю дхяйперхгюжхх (цЖ)
Fs=1000 * (mod(Nb,5)+1);
disp(Fs)
disp('A1 = ');      % юлокхрсдш дхяйпермшу цюплнмхй
A1=0.8+0.01*Nb;
disp(A1)
disp('A2 = ');   
A2=A1*1.5;
disp(A2)
disp('f1 = ');      % вюярнрш дхяйпермшу цюплнмхй (цЖ)
f1=Fs/8;
disp(f1)
disp('f2 = ');    
f2=2*f1;
disp(f2);
disp('sigma = ');% бейрнп гмювемхи яйн ьслю
sigma=[0,A1,A1*2,A1*4];
disp(sigma);
