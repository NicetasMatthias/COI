...
N = 30 + mod(Nb,5);            % ÄËÈÍÀ ÏÎÑËÅÄÎÂÀÒÅËÜÍÎÑÒÈ
disp('N = ');              % ÄËÈÍÀ ÏÎÑËÅÄÎÂÀÒÅËÜÍÎÑÒÈ
disp(N);
T = 0.0005*(1+mod(Nb,3));              % ÏÅĞÈÎÄ ÄÈÑÊĞÅÒÈÇÀÖÈÈ
disp('T = ');              
disp(T);              
a = (-1)^Nb * (0.8+0.005*Nb);              % ÎÑÍÎÂÀÍÈÅ ÄÈÑÊĞÅÒÍÎÉ İÊÑÏÎÍÅÍÒÛ
disp('a = '); 
disp(a); 
C = 1+mod(Nb,5);      % ÀÌÏËÈÒÓÄÀ ÄÈÑÊĞÅÒÍÎÃÎ ÃÀĞÌÎÍÈ×ÅÑÊÎÃÎ ÑÈÃÍÀËÀ
disp('C = ');
disp(C);
w0 = pi/(6+mod(Nb,5));    % ×ÀÑÒÎÒÀ ÄÈÑÊĞÅÒÍÎÃÎ ÃÀĞÌÎÍÈ×ÅÑÊÎÃÎ ÑÈÃÍÀËÀ
disp('w0 = ');
disp(w0);
m = 5+mod(Nb,5);              % ÂÅËÈ×ÈÍÀ ÇÀÄÅĞÆÊÈ
disp('m = '); 
disp(m); 
U = Nb;              % ÀÌÏËÈÒÓÄÀ ÈÌÏÓËÜÑÀ
disp('U = ');
disp(U);
n0 = mod(Nb,5)+3;            % ÌÎÌÅÍÒ ÍÀ×ÀËÀ ÈÌÏÓËÜÑÀ
disp('n0 = ');
disp(n0);
n_imp = mod(Nb,5)+5;      % ÄËÈÒÅËÜÍÎÑÒÜ ÈÌÏÓËÜÑÀ
disp('n_imp = '); 
disp(n_imp); 
B(1) = 1.5 + mod(Nb,5);              % ÂÅÊÒÎĞ ÀÌÏËÈÒÓÄ
B(2) = 5.7 - mod(Nb,5);
B(3) = 2.2 + mod(Nb,5);
disp('B = ');
disp(B);
w(1) = pi/(4 + mod(Nb,5));              % ÂÅÊÒÎĞ ×ÀÑÒÎÒ 
w(2) = pi/(8 + mod(Nb,5));  
w(3) = pi/(16 + mod(Nb,5));  
disp('w = '); 
disp(w); 
A(1) = 1.5 - mod(Nb,5);        % ÂÅÊÒÎĞ ÊÎİÔÔÈÖÈÅÍÒÎÂ ËÈÍÅÉÍÎÉ ÊÎÌÁÈÍÀÖÈÈ
A(2) = 0.7 + mod(Nb,5);
A(3) = 1.4 + mod(Nb,5);
disp('A = ');
disp(A);
Mean = mod(Nb,5) + 3;  % ÇÀÄÀÍÍÎÅ ÌÀÒÅÌÀÒÈ×ÅÑÊÎÅ ÎÆÈÄÀÍÈÅ ØÓÌÀ
disp('Mean = ');
disp(Mean);
Var = mod(Nb,5) + 5;    % ÇÀÄÀÍÍÀß ÄÈÑÏÅĞÑÈß ØÓÌÀ 
disp('Var = ');
disp(Var);
